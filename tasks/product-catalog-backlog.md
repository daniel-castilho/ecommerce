...
**Status:** ✅ IMPLEMENTED + QA'd (2026-07-31) — `ManageProductBean` (`@ViewScoped`, `@RolesAllowed("ADMIN")`) + `manageProduct.xhtml`; WAR builds. Admin enforced two ways: bean-level `@RolesAllowed("ADMIN")` **and** a session-role page guard (`#{userBean.hasRole('ADMIN')}`) — the latter originally the only container-independent gate (no container IdentityStore at the time); since 2026-08-01 both are backed by real container RBAC (`UserIdentityStore` + `LoginAuthenticationMechanism`, see `docs/lessons.md` #8). Image management (upload, alt text, primary radio, reorder) backed by the new `UpdateProductImageUseCase` port added to `ProductApplicationService`. **Manual QA passed against LocalStack** (sequence doc Step 7): create, duplicate-SKU rejection on the field, image upload/alt-text/primary, category assignment, publish → `ACTIVE`. Non-admin session blocked by the page guard.

**Acceptance criteria:**
- [x] **Given** the admin fills in name, price, SKU, and at least one image, **when** the form is submitted, **then** a product is created in `DRAFT` status and the admin is redirected to the product's edit page with a success message. — *create → DRAFT → redirect to edit page; the create form gained an optional image upload (2026-08-01), closing the previous deviation — the uploaded file becomes the product's primary image on the redirect to the edit page. If the image is rejected, the product is still created and a warning names the reason.*
