...
**Audience:** this document is written for an AI coding agent that will implement the changes directly in the `java-ee-online-shop` repository. It assumes the stack and conventions already established in the project's `coding-standards.md` (Jakarta EE 10/11 migration in progress, multi-module Maven layout, CDI/EJB, JPA, JSF/Facelets, `java.util.logging`, Lombok available) **and the project's adopted target architecture: hexagonal (ports & adapters).**

**Status:** draft for implementation. Section 14 lists assumptions the implementer should flag back to the human if they turn out to be wrong. See the companion documents for backlog breakdown (`product-catalog-backlog.md`) and step-by-step build order (`product-catalog-implementation-sequence.md`).

> ## ⚠️ WRITTEN FOR THE LEGACY `java-ee-online-shop` REPO — NOT YET IMPLEMENTED HERE (July 30, 2026)
>
> The body assumes a repo layout that does **not** exist in this project: `catalog-core`/`catalog-adapters`/`catalog-web`, `ProductDao`/`ProductEjb`/`CategoryCacheEjb`, `@Stateless`, GlassFish, Lombok. This repo is the `ecommerce` monolith. Where the body conflicts with the box below, **the box wins**.
>
> **Real repo (`/home/castilho/projects/ecommerce`):** single Maven module `product-catalog` (package root `com.loja.productcatalog`) with hex packages `domain/model`, `domain/port/in`, `domain/port/out`, `application/service`, `application/dto`, `adapter/in/web`, `adapter/out/persistence`. The `user-account` module is the reference for every convention (ports in `domain/port`, `@ApplicationScoped` + `@Transactional`, only `domain` framework-free, thin JSF beans, Strategy for conditional logic, CDI observers for events).
>
> **Current as-built state of `product-catalog`:** `Product` (id `String` UUID, name, description, `Money` price, stock, `reserveStock()`), `ProductJpaEntity` (table `tb_product`), `ProductRepositoryPort` (`findByName`/`findAll`/`findById`/`save`), `SearchProductsService`, `ProductCatalogBean` + `web/.../webapp/product-catalog/catalog.xhtml`. No `Sku`/`Slug`/status/images/category, no pagination, no admin CRUD, **no tests and no ArchUnit test in this module yet**. `order-checkout` consumes `ProductRepositoryPort` + `Product.reserveStock` (`CheckoutService`) — any change to the domain/port must keep that compiling.
>
